...
parser.add_argument('-r', '--remote_folder', nargs='?', default='Downloads', help='Folder name in your Dropbox')
parser.add_argument('-l', '--local_rootdir', nargs='?', default='~/Downloads', help='Local directory to upload')
parser.add_argument('--token', default=None, help='token is mandatory. Access token ''(see https://www.dropbox.com/developers/apps)')
...
def main():
    ...
    if not args.token:
        ...
        # OAuth2 access token
        args.token = os.environ["DBX_TOKEN"]

    remote_folder = args.remote_folder
    rootdir = os.path.expanduser(args.rootdir)
    print('Dropbox remote folder name:', remote_folder)
    ...
    sync_folder(dbx, remote_folder, rootdir, args)
    
    
def sync_folder(dbx, folder, rootdir, args):
    """
    Args:
        folder: remote folder on dropbox
